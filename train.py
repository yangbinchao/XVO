...
import model
from loss.ms_loss import multi_scale_loss
from utils.logger import TermLogger, AverageMeter
import utils.custom_transforms as custom_transforms
from utils.compute_error import compute_depth_errors, compute_pose_errors
from utils.inverse_warp import pose_vec2mat
from utils.comprehensive import tensor2array, save_checkpoint, save_path_formatter, log_output_tensorboard
...
parser.add_argument('-p', '--photo-loss-weight', type=float, help='weight for photometric loss', metavar='W', default=0.5)  # 1  0.85
...
def main():
    ...
    if args.dataset_format == 'stacked':
        from data.stacked_sequence_folders import SequenceFolder
    elif args.dataset_format == 'sequential':
        from data.sequence_folders import SequenceFolder
    save_path = save_path_formatter(args, parser)
    args.save_path = 'ckpts'/save_path
    ...
    '''
    加载验证数据集，建议打开真值验证模式
    如果没有可用的 Groundtruth，则验证集与训练集使用测量重建造成的光度损失
    '''
    if args.with_gt:
        if args.with_pose:
            from data.validation_folders import ValidationSetWithPose
    ...
    '''
    if mixed_precision:
        print("=> using mixed precision train model")
        disp_net, optimizer = amp.initialize(disp_net,optimizer, opt_level="O1")
        pose_exp_net, optimizer = amp.initialize(pose_exp_net, optimizer, opt_level='O1')
    '''
    实验记录
    '''
    with open(args.save_path/args.log_full, 'w') as csvfile:
        writer = csv.writer(csvfile, delimiter='\t')
        writer.writerow(['train_loss', 'loss_l1', 'loss_ssim', 'loss_fsim', 'loss_smooth'])
    ...
    for epoch in range(args.epochs):
        ...
        # 由您选择最相关的错误来衡量您的模型的性能，注意一些衡量标准是最大化（例如 a1,a2,a3）
...
def train(args, train_loader, disp_net, pose_exp_net, optimizer, epoch_size, logger, tb_writer):
    global n_iter, device, w1, w2, w3, w4
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter(precision=4)

    # print("******************[INFO]******************")
    # print("photo_loss_weight: {}, mask_loss_weight: {}, smooth_loss_weight: {}",format(w1,w2,w3,w4))
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(train_loader):
        ...
        loss_l1, loss_ssim, loss_fsim, loss_smooth, warped, diff = multi_scale_loss(tgt_img, ref_imgs, intrinsics,
                                                               depth, pose,
                                                               args.rotation_mode, args.padding_mode)

        loss = w1*loss_l1 + w2*loss_ssim + w3*loss_fsim + w4*loss_smooth

        if log_losses:
            tb_writer.add_scalar('loss_l1', loss_l1.item(), n_iter)
            tb_writer.add_scalar('loss_ssim', loss_ssim.item(), n_iter)
            tb_writer.add_scalar('loss_fsim', loss_fsim.item(), n_iter)
            tb_writer.add_scalar('loss_smooth', loss_smooth.item(), n_iter)
            tb_writer.add_scalar('total_loss', loss.item(), n_iter)

        if log_output:
            tb_writer.add_image('train Input', tensor2array(tgt_img[0]), n_iter)
            for k, scaled_maps in enumerate(zip(depth, disparities, warped, diff)):
        ...
        with open(args.save_path/args.log_full, 'a') as csvfile:
            writer = csv.writer(csvfile, delimiter='\t')
            writer.writerow([loss.item(), loss_l1.item(), loss_ssim.item(), loss_fsim.item(), loss_smooth.item()])
...
def validate_with_gt_pose(args, val_loader, disp_net, pose_exp_net, epoch, logger, tb_writer, sample_nb_to_log=3):
    ...
    for i, (tgt_img, ref_imgs, gt_depth, gt_poses) in enumerate(val_loader):
        ...
        output_poses = pose_exp_net(tgt_img, ref_imgs)
        ...
        if log_outputs and i in batches_to_log:  # log first output of wanted batches
            ...
            log_output_tensorboard(tb_writer, 'val', index, '', epoch, output_depth, output_disp, None, None)
...
def validate_without_gt(args, val_loader, disp_net, pose_exp_net, epoch, logger, tb_writer, sample_nb_to_log=3):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(val_loader):
        ...
        loss_1, loss_ssim, warped, diff = photometric_reconstruction_loss(tgt_img, ref_imgs,
                                                               intrinsics, depth,
        ...
        loss = (1-w1)*loss_1 + w1*loss_ssim + w2*loss_2 + w3*loss_3
        losses.update([loss, loss_1 + loss_ssim, loss_2])
